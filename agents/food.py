...
llm = ChatOpenAI(
    ...
    temperature=0.5,                       # 결과 다양성 조절, 너무 높으면 비일관적 즉, 피자와 김치찌개가 같이 나올 수 있음
    model_kwargs={
...
def recommend_food(state: dict) -> dict:
    ...
    # GPT에게 보낼 프롬프트 정의 (f-string 내부 문자열은 안전하게 작성), 페르소나를 구체적으로 작성할 것, 페르소나 구체성 여부에 따라 답변이 많이 달라짐짐
    # "또는 어울리는" 부분은 사용자가 특정 음식을 언급했을 때 그 음식과 관련된 다른 음식을 추천하도록 유도하는 부분입니다. 이 문장 여부에 따라 추천의 질이 달라질 수 있습니다.

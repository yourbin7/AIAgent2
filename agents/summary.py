...
llm = ChatOpenAI(
    ...
    temperature=0.7                         # 창의성 높임 (감성적 문장 유도, 랜덤성만 높아짐), 낮을수록 즉, 0에 가까운 경우는 냉정한 어투, 보통 1정도로 설정
)                                           # 와인 추천, 투자보고서 관련된 에이전트 생성에서 temperature를 낮게 또는 높게한다..부분은 신중하게 접근 
...
def summarize_message(state: dict) -> dict:
    ...
    # GPT 프롬프트 구성 : 4가지 구조적 정보로 구성해야 함(여기에서는 미흡)
    prompt = f"""

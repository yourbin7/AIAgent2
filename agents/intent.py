...
llm = ChatOpenAI(
    ...
    }          # OPENAI API 1.6.0 이상에서만 사용 가능, LANGCHAIN_OPENAI 0.0.4 이상에서만 지원
    # response_format="json" 으로 사용가능하나, 가능하면 위의 코드로 사용할 것            # JSON 배열로 응답 (deprecated)
)

def classify_intent(state: dict) -> dict:
    """사용자의 입력 문장을 기반으로 GPT를 호출하여 food, activity, unknown 중 하나의 intent를 분류합니다."""
    # 일반노드이든 툴노드이든 docstring 으로 표현해주어야 함 <== 윗 줄 문장
    user_input = state.get("user_input", "")  # 사용자의 입력 문장 추출

    # GPT에게 의도를 분류하도록 요청할 프롬프트, 첫 줄은 항상 페르소나 기재, 바로 아래 문장
    # 조금 애매한 표현이라도 의미가 보이면 food 또는 activity로 분류하세요. <== 이 한줄로 성능을 많이 끌어올렸음
    # 리스트(배열), 딕션너리(객체) 형태로 응답할 수 있도록 프롬프트를 작성 
    prompt = f"""
...
"""
    ...
    try:
        ...
        if isinstance(parsed, list) and parsed and parsed[0] in ["food", "activity"]:
            return {**state, "intent": parsed[0]}   # 키워드 unpack operator 사용 <== **state
